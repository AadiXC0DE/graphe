...
**The coding agent with a real workspace around it.**

pi is a terminal agent. Graphe is what it becomes with a real interface: projects on a shelf, conversations in tabs, every turn a version you can go back to, and a safety engine that asks in plain words before anything risky. Built on pi, the layer above is all Graphe's.
...
pi in a real window: projects on a shelf, conversations in tabs, a history rail, light and dark, and the model and the money where you can see them.

**02 · Version history**
Every change is versioned the moment it happens. Going back is one press, and going back is itself undoable.
...
**11 · MCP: bring your own tools**
Your MCP servers plug in through one file: tools start on first use, stop with the session, and every call asks first.

**12 · Conversations side by side**
Two conversations can be open at once. A chat works in the project folder and only one run writes there at a time, so the second waits its turn rather than writing over the first. Ask for a worktree and it gets a copy of its own with its own branch, running beside the rest and merged back when you say so.

**13 · Background work**
Send a piece of work away and come back to the list: what finished, what did not, and what is waiting on you. A job that is still going says which step it is on.

**14 · PRD to shipped change**
...
**17 · Pull requests**
A conventional branch name, a title that matches it, and the change described in the body, posted with the `gh` you already have signed in.

**18 · Usage & cost tracking**
...
**24 · A browser it can actually drive**
...
**25 · Working the computer itself**
...
**28 · Why the page is blank, without asking**
...
**29 · It works the computer without taking your mouse**
...
**31 · Things this project always does**
...
**34 · Two models: one thinks, one does**
...
- **Agent core:** pi (`@earendil-works/pi-coding-agent`). Graphe is the layer above it, safety, code review, version history, money, memory, and the interface, and one module owns every Pi import, so upgrades break one file, not fifty.
