...
| 4 | The busy rectangle / loading row | **Done.** `arriving(holding)` in `src/App.tsx` with `role="status"` and `aria-label` at all 11 call sites, and the loading row is in `scripts/visual-matrix.mjs`: it holds the Settings chunk 3 s on the `--built` run, asserts the rectangle is drawn, named and inside the window, and that it is gone once the sheet arrives. |
| 5 | Person-only visual checks | **Not done.** Needs the owner, ~40 minutes. |
| 6 | Phase 4 residue | **Done.** Draft proof, session states, registry-side view records (v2 index), S11 address mapping, S12 rapid New presses, the run note's lifetime and `workspaceId`. The window's half of view records landed: `viewsLook(where)` and `viewsNote(shown, where)` in `src/lib/ipc.ts`, `electron/preload.ts`, `electron/main.ts` and the mock bridge, with `src/App.tsx` writing the whole pane set 400 ms held back and reading it at launch through `panesFrom`; `tests/panes.test.ts` 29. |
