...
| 6 | Phase 4 residue, item 6 of `docs/graphe-stabilization-remaining-2026-09-16.md`: the never-sent chat's draft (`tests/draft-new-chat.test.ts`, 3 tests), the session states (`tests/session-states.test.ts`, 19 tests), the registry half of view records (`tests/workspace-registry.test.ts`, 35 tests) and S11's address mapping (`tests/address-mapping.test.ts`, 3 tests) are done; view records still need the window's `noteView`/`viewInPane` call, which is the App.tsx owner's, and S12, the interrupted note's lifetime and `workspaceId` in the run record are still open | 4 | open, four rows done |
