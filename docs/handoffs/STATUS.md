...
| 6 | Phase 4 residue, item 6 of `docs/graphe-stabilization-remaining-2026-09-16.md`: the never-sent chat's draft is done and proven (`tests/draft-new-chat.test.ts`, 3 tests); the session states are done (`waiting-input` and `compacting` driven from Pi's own events, `archived` documented as the record flag it is; `tests/session-states.test.ts`, 19 tests); view records are done in the registry (`views` on the index, schema version 2 read forward from 1, `tests/workspace-registry.test.ts` 35 tests) with only the window's `noteView`/`viewInPane` call left, which is the App.tsx owner's; still open are the four small rows | 4 | open, three rows done |
