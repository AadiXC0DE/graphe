...
| T21 | One runtime, writer and bill across two panes showing one chat | Half proven. The pane half is now built and tested: `tests/panes.test.ts` `one conversation in two panes` > "is one conversation with two views, not two conversations" (two panes naming one address are two views, and nothing there could ask for a second runtime because a pane holds an address and not a session), with `tests/panes-render.test.ts` and `tests/tab-strip-split.test.ts` for the press and the band. One runtime behind them is structural rather than asserted: `Sessions` is keyed by conversation (`src/domain/conversations.ts:221`). Not provable today: one runtime, one transcript and one *bill* through a real window, and the durable half — `electron/services/workspace-registry.ts` has no `views` at all, so a view id lives only in window state |
| T22 | Composer, shortcuts and inspector address the pane in focus | Half proven. The composer and the inspector do: `tests/panes.test.ts` "holds the panes and reads the controls off the focused one", "draws the pane beside from the same desk, so no second session is built", "binds the beside pane's cards to its own conversation" and "says which chat the panel is pinned to, and offers the pin" assert over `src/App.tsx`; `tests/panes-render.test.ts` drives the band's own keyboard (Arrow keys, one tab stop, closing the pane it names). Not provable: the same through a real window, and the global shortcuts — the pane band is a tablist and the shell hands Escape back (`CHANNEL.paneKey`, `electron/main.ts:1083`), but nothing drives a shortcut landing in the unfocused pane |
