...
(tool names; custom renderers done 2026-09-16), E10 done, E11 open (the bundled
route), E12 open. E02's last step — the terminal inside a packaged app — landed
2026-09-16. Read again on this tree on 2026-09-15.
...
including the gaps it finds (an abort signal that is not carried, no status
line, a plain text widget refused with the component form, no duration on a tool
record, commands that never reach the picker, shortcut conflicts nobody reports,
a provider the window does not list — and two that are now closed: a notice
carries the add-on's name, read off the call stack, and is written into the
conversation's own record so reading it back brings it, `tests/addon-notice.test.ts`,
7). The terminal-mode criterion is not met: terminal compatibility
