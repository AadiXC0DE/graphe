...
// Finder, so nothing is translocated or asked about), a real provider, and the
// terminal.
//
...
// asserts the window says so. On this build the window never does — the log
// records `git here=false` and the screen says nothing — which is what the flag
// is for. The npm half of that is not testable here at all: the app widens its
// own PATH to include Homebrew, and this machine has npm there.
...
console.log(
  'Not proven here: signing and notarization beyond verify-package\'s ad-hoc check, the x64\n' +
    'bundle, Gatekeeper and quarantine (Finder was not used, so nothing was translocated),\n' +
    'a real provider, and the terminal. --without-git checks the window for the git-missing\n' +
    'notice: the log records it, the window does not show it, and that is a fault rather than\n' +
    'a pass. That every Git action is then disabled, and that files and chat still work\n' +
    'without it, is not checked. Missing npm cannot be tested on this machine at all: the app\n' +
    'widens its own PATH to Homebrew (electron/main.ts widenPath), where npm is.',
);
