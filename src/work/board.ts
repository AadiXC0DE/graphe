...
export type WorkState = 'waiting' | 'running' | 'done' | 'failed';
...
export type BandKey = 'running' | 'waiting' | 'finished';
...
const BAND_ORDER: readonly BandKey[] = ['running', 'waiting', 'finished'];
...
export function roomLeft(pieces: readonly OnBoard[], most: number = AT_A_TIME): number {
  return Math.max(0, most - howManyGoing(pieces));
}
...
export function saysDrop(state: WorkState): string {
  return state === 'running' || state === 'waiting' ? boardWords.stop : boardWords.drop;
}
