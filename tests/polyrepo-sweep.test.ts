 ...
 *
 *  Source text, not behaviour: how a several-project folder is resolved in the shell and named by the window above; electron/main.ts sits behind IPC and App.tsx needs the whole window, so neither join has anything a test can call. */
...
describe('the window names the project it is acting on', () => {
  ...
  it('the band at the foot acts on the project the panel is showing', () => {
    expect(OVERVIEW).toContain('onPutBack={(versionId) => onPutBack(versionId, whose?.name)}');
  });
